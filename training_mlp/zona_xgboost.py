...
XGBoost meta-labeling for the Zona strategy — trained separately per direction.

Outputs saved to strategies/zona_strat/model/{long,short}/:
  model_xgb_best.json
  scaler_xgb_best.pkl
  config_xgb.json
  results_cv_xgb.csv
...
def plot_cv_summary(results, naive_acc, out_dir, label):
    ...
    fig.tight_layout()
    path = out_dir / "cv_summary_xgb.png"
    fig.savefig(path, dpi=100)
    plt.close(fig)
    print(f"  Saved -> {path}")
...
def train_direction(df_dir: pd.DataFrame, direction_label: str, out_dir: Path):
    ...
    if fold_size is not None:
        fold_size = fold_size // 2  # half since we split by direction
    ...
    print(f"\n  Total={len(X):,}  CV={len(X_cv):,}  Test={len(X_test):,}")
    ...
    print(f"  Best fold: {best_fold_idx}  (val AUC {best_auc:.4f})")
    print(f"  Naive acc: {naive_acc:.4f}")

    plot_feature_importance(best_model, FEATURE_COLS, out_dir, direction_label)
...
def run():
    print("=" * 52)
    print("  ZONA XGBoost — DIRECTION SPLIT")
    print("=" * 52)
    ...
    for direction_val, direction_label, subdir in [
        ...
        train_direction(df_dir, direction_label, out_dir)
