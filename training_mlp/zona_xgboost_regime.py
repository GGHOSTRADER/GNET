"""
zona_xgboost_regime.py
======================
XGBoost meta-labeling trained separately per macro regime.

Outputs saved to strategies/zona_strat/model/regime/{regime_name}/:
  model_xgb_best.json
  scaler_xgb_best.pkl
  config_xgb.json
  results_cv_xgb.csv
...
BASE_DIR = Path("strategies/zona_strat/model/regime")
...
def train_regime(df_reg: pd.DataFrame, regime_label: str, out_dir: Path):
    out_dir.mkdir(parents=True, exist_ok=True)

    X = df_reg[FEATURE_COLS].values.astype(np.float32)
    y = df_reg["Label"].values.astype(np.float32)
    ...
    print(f"\n  -- CV Summary ({regime_label}) --")
    print(res_df.to_string(index=False))
    print(f"  Best fold: {best_fold_idx}  (val AUC {best_auc:.4f})  naive_acc: {naive_acc:.4f}")

    plot_feature_importance(best_model, FEATURE_COLS, out_dir, regime_label)
    plot_cv_summary(results, naive_acc, out_dir, regime_label)
...
def run():
    print("=" * 55)
    print("  ZONA XGBoost -- REGIME SPLIT")
    print("=" * 55)
    ...
    summary = []
    for col, subdir in REGIMES:
        print(f"\n{'='*55}")
        print(f"  Regime: {subdir}  ({(df[col]==1).sum():,} trades)")
        print(f"{'='*55}")
        df_reg  = df[df[col] == 1].reset_index(drop=True)
        if len(df_reg) < 200:
            print(f"  Skipping — too few trades ({len(df_reg)})")
            continue
        out_dir = BASE_DIR / subdir
        best_auc, naive_acc, n_test = train_regime(df_reg, subdir, out_dir)
        summary.append({"regime": subdir, "trades": len(df_reg),
                         "best_auc": best_auc, "naive_acc": naive_acc, "n_test": n_test})

    print(f"\n{'='*55}")
    print("  SUMMARY")
    print(f"{'='*55}")
    print(f"  {'Regime':<25} {'Trades':>7} {'Best AUC':>9} {'Naive Acc':>10} {'Test N':>7}")
    print(f"  {'-'*55}")
    for s in summary:
        print(f"  {s['regime']:<25} {s['trades']:>7,} {s['best_auc']:>9.4f} {s['naive_acc']:>10.4f} {s['n_test']:>7,}")
