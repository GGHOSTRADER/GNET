...
6. Align the live VP adapter with the agreed just-in-time rule now implemented offline: publish refreshed previews during the final second, but commit interval-history state only once using the freshest preview. This preserves low latency and correct 30-second semantics for recent delta, VA expansion, POC migration, and POC velocity.
...
12. Add Redis/TCP integration tests for candidate ingestion, routing, decision publication, reconnection, malformed payloads, consumer-group recovery, and service restarts.
...
17. Align the live scikit-learn version with the 1.8.0 version used to serialize `scaler_best.pkl`, or regenerate and verify the scaler under the deployment environment. The current 1.5.1 runtime emits an incompatibility warning.
18. After installing the staged hardened DLLs and recompiling the updated EasyLanguage documents, run a deliberate Python-outage test and confirm TradeStation remains responsive while ports 9009–9012 are unavailable and later reconnects cleanly.
